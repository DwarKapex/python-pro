...
class ArgumentsField(RequiredNullable):
    def validate_value(self, value) -> Tuple[str | None, int]:
        if not isinstance(value, dict):
            return (
                f"Invalid arguments type: expected <dict>, recieved {type(value)}",
                INVALID_REQUEST,
            )
        return None, OK
...
def method_handler(
    request: Dict[str, Union[str, Dict]], ctx: Dict[str, Union[str, int]], store: str
) -> Tuple[str | None, int]:
